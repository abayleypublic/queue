...
@mcp.tool
def add_to_queue(
    id: Annotated[int, "The ID of the queue"],
    entity_id: Annotated[int, "The ID of the entity to add to the queue"],
...
def remove_from_queue(
    id: Annotated[int, "The ID of the queue"],
    entity_id: Annotated[int, "The ID of the entity to remove from the queue"]
    ) -> str:
    """
    ...
    with insecure_channel(cfg.backend.url) as channel:
        ...
        queue: List[Entity] = []
        try:
            response: GetQueueResponse = stub.GetQueue(GetQueueRequest(id=id))
            for entity in response.entities:
                if entity.id != entity_id:
                    queue.append(entity)
        except RpcError as e:
            logger.error("failed to remove from queue: " + str(e))
